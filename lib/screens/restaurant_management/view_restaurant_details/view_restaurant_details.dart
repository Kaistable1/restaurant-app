...
class ViewRestaurantDetails extends StatefulWidget {
  ViewRestaurantDetails({super.key});

  @override
  State<ViewRestaurantDetails> createState() => _ViewRestaurantDetailsState();
}

class _ViewRestaurantDetailsState extends State<ViewRestaurantDetails> {
  final drawerController = Get.put(DrawerControllerX());

  final resturantController = Get.put(RestaurantManagementController());

  final addRestaurantController = Get.put(AddRestaurantTabController());

  final fillDataController = Get.put(FillDataRestaurantController());
        ...
        StreamBuilder(
            ...
            builder: (context, snapshot) {
              ...
              RestaurantModel restaurant =
                  RestaurantModel.fromMap(restaurantData.data()!);
...
class OperatingHoursContainer extends StatelessWidget {
  ...
  @override
  Widget build(BuildContext context) {
    return StreamBuilder<List<MyOperatingHours>>(
      stream: controller.getOperatingHours(auth.currentUser?.uid ?? ''),
      builder: (context, snapshot) {
        if (snapshot.hasError) {
          return Container(
